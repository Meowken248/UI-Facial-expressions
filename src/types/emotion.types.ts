...
export interface BrowConfig { rotation?: number; translateX?: number; translateY?: number; curve?: 'soft' | 'raised' | 'sad' | 'angry' }
export interface MouthConfig { shape: 'smile' | 'small-smile' | 'wide-smile' | 'laugh' | 'open' | 'o' | 'sad' | 'frown' | 'tongue' | 'wave' | 'sleep' | 'squiggly'; translateY?: number; scaleX?: number }
...
export interface EmotionConfig { leftEye: EyeConfig; rightEye: EyeConfig; leftBrow: BrowConfig; rightBrow: BrowConfig; mouth: MouthConfig; cheeks?: { visible: boolean; intensity?: number }; effects?: EffectConfig[]; faceAnimation?: 'none' | 'bounce' | 'breathe' | 'shake' | 'pulse' }
